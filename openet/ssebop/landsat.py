...
def ndwi(landsat_image):
    ...
    #   since C02 surface reflectance values can be negative
    #   but the normalizedDifference function will return nodata
    # changed the ndwi_img = landsat_image.max(0).normalizedDifference(['swir1', 'green']) to now:
    ndwi_img = landsat_image.max(0).normalizedDifference(['green', 'nir'])

    #changed b1 and b2 corresponding bands as well following ndwi
    b1 = landsat_image.select(['green'])
    b2 = landsat_image.select(['nir'])
